...
def open_win1():
    ...
    tk.Label(win1,text ="Enter the Amount :").grid(row=1)
...
def open_win3():
    ...
    def open_win301():
        win301 = tk.Toplevel(win3)
        win301.title("Transaction based on type")
        win301.geometry("500x600")

        result = pft.search_by_type(transactions,e1.get(),info_callback = lambda msg : messagebox.showinfo("Info",msg))
...
def open_win4():
    ...
    def open_win401():
        win401 = tk.Toplevel(win4)
        win401.title("Transaction based on Description")
        win401.geometry("500x600")

        result = pft.search_by_desc(transactions,e1.get(),info_callback = lambda msg : messagebox.showinfo("Info",msg))
...
tk.Button(btn_frame,text ="Search by Type",command= open_win3).grid(row = 0,column = 2,padx = 10)
tk.Button(btn_frame,text ="Search by Description", command = open_win4).grid(row=1,column = 0,padx = 10, pady = 10)
